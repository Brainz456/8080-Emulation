...
int disassemble(unsigned char *codeBuffer, int pc){
    ...
    switch(*opcode){
        ...
        case 0x22: printf("SHLD    #$%02x%02x",opcode[2],opcode[1]);opbytes=3; break; // This one is a bit odd, adr is the next two bytes
        ...
        case 0x2a: printf("LHLD    #$%02x%02x",opcode[2],opcode[1]); opbytes=3; break; // This one is also a bit odd, adr is the next two bytes
        ...
        case 0x32: printf("STA    #$%02x%02x",opcode[2],opcode[1]); opbytes=3; break; // This one is also a bit odd, adr is the next two bytes
        ...
        default: printf("Error - NIY:0x%02x",opcode[0]); break;
    }
    printf("\n");
    return opbytes;
}
